...
from util.dataloader import Dataset_CRNN
...
# set path
train_path = "./data/training_no_junk/"
valid_path = "./data/Validation_Normal_Low_Grade/"
save_model_path = "./models/CRNN/"

# EncoderCNN architecture
CNN_fc_hidden1, CNN_fc_hidden2 = 1024, 768
CNN_embed_dim = 512      # latent dim extracted by 2D CNN
img_x, img_y = 512, 510  # resize video 2d frame size
...
# training parameters
k = 2             # number of target category #cancer (1) or normal (0)
...
def train(log_interval, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (X, y) in enumerate(train_loader):
        ...
        if (type(y.cpu().data.squeeze().numpy()) == list):
...
train_set, valid_set = Dataset_CRNN(
    train_path, transform=transform), Dataset_CRNN(valid_path, transform=transform)
